...
date: 2023-12-02
weight: 2
keywords: ["veScale", "MegaScale"]
---

import output from '../../public/blog/megascale/output.png';
import output_1 from '../../public/blog/megascale/output_1.png';
import output_2 from '../../public/blog/megascale/output_2.png';
import output_3 from '../../public/blog/megascale/output_3.png';
import output_4 from '../../public/blog/megascale/output_4.png';
import output_5 from '../../public/blog/megascale/output_5.png';
import output_6 from '../../public/blog/megascale/output_6.png';
import output_7 from '../../public/blog/megascale/output_7.png';
import output_8 from '../../public/blog/megascale/output_8.png';
import output_9 from '../../public/blog/megascale/output_9.png';
import output_10 from '../../public/blog/megascale/output_10.png';
import output_11 from '../../public/blog/megascale/output_11.png';
import output_12 from '../../public/blog/megascale/output_12.png';
import output_13 from '../../public/blog/megascale/output_13.png';
import output_14 from '../../public/blog/megascale/output_14.png';
import output_15 from '../../public/blog/megascale/output_15.png';
import output_16 from '../../public/blog/megascale/output_16.png';

# Integration of Benchmark Testing, Optimization, and Ecosystem Collaboration for AI ASICs

Date: 2023-12-02

From September 26-28, the MegaScale + CloudNativeCon, hosted by the Linux Foundation and CNCF, was held in Shanghai. As active contributors and end-users in the community, the ByteDance and Volcano Engine teams made 7 presentations at the conference — MegaScale 2023 | How ByteDance Builds Cloud-Native Infrastructure for AI. This series of content is organized based on the conference presentations, and we welcome your attention.

<img src={output} alt="output"></img>

veScale, open-sourced by ByteDance, is a solution for "integrating evaluation, optimization, and ecosystem collaboration for AI ASICs."
...
- The second part explains the motivation behind veScale, especially considering the high reputation of MLPerf in the industry;
...
In the second part, one point was not mentioned — with MLPerf already existing, why create veScale? Simply put, MLPerf is difficult to meet actual business evaluation needs. Here's a simple comparison:

Firstly, the evaluation perspectives differ. veScale is initiated purely from the user's perspective, while MLPerf is hosted by a supplier committee, leading to different focuses.

Secondly, in terms of fairness, veScale does not require apple-to-apple alignment but is result-oriented, accepting manufacturers' proprietary technologies.

Then, veScale's evaluation set updates faster, closely following business needs and SOTA models. When we notice that widely used models in business are evolving, we will update them promptly and notify manufacturers.

Lastly, veScale will restrict the way of evaluation access according to business usage, abstracting the usage method into an API, defining the API return format, but not the implementation process of the API.
...
Firstly, transparency and reproducibility. Open source goes without saying, but transparency is demanded because we require manufacturers not only to provide evaluation results but also to offer a reproducible environment. This includes how to achieve the evaluation results under the veScale framework. This ensures the reproducibility of the evaluation results as much as possible, and similar results can be obtained when similar models are used in actual business scenarios.

Secondly, tailored for AI production scenarios. This means that veScale's evaluation process is designed based on actual production scenarios. To some extent, veScale is not just an evaluation suite; it can actually be used as a production tool and integrated into the production process.

Thirdly, keeping pace with business needs and state-of-the-art (SOTA) technology. To ensure consistency with the latest technologies and business requirements, veScale continually updates its benchmark tests to reflect current business scenario needs and cutting-edge technology, providing users with the latest and most relevant performance evaluations. This also allows manufacturers to stay informed about which models are widely used.

<img src={output_9} alt="output_9"></img>

To address the issue of difficult choices, veScale has collected information on commonly available hardware products in the market and summarized them, as seen in this image. Of course, the overview does not include all products available in the market, as veScale filters out some immature hardware and software products through its evaluation access constraints and report submission thresholds.
...
We hope to reduce the risk of unpredictability through comprehensive evaluation sets. As mentioned earlier, to ensure alignment with the latest technologies and business requirements, veScale continuously updates its benchmark tests. This can be simply divided into three parts:

- Regular Benchmark Models. In the reporting standards of veScale, manufacturers are required to submit reports for at least 5 different models, including regular models which are mandatory. If these regular models are not well supported, they do not meet the entry requirements of veScale.
...
We welcome everyone to submit models of interest to veScale!

<img src={output_12} alt="output_12"></img>

According to the business scenarios we observed, the AI business production process is abstracted into veScale's evaluation framework. Let's take a look at veScale's framework, which mainly includes Task Zoo, Compile Backend, and Runtime Backend.
...
veScale's design imitates the stages of model export to database, optimization, compression, and model operation. Here, the Task Zoo can be compared to a model library, Compile Backend can be understood as a model optimization and compression tool, and Runtime Backend is analogous to model operation.

From the evaluation process on the right side, you can see that veScale's model evaluation simplifies the actual deployment stages of model conversion and performance stress testing. This method ensures high reproducibility of evaluation results, and when migrated to actual business scenarios, there won't be significant deviations.
...
veScale also hopes to act as a bridge between businesses and manufacturers, making the implementation of ASICs smoother.
...
with non-sparsified ones? The reason is that veScale is result-oriented and open to innovative technologies. Specifically for this case, whether mixed precision and sparsification are acceptable should not be denied simply because others haven't used them. In other words, using such optimization methods is not inherently wrong; perhaps the manufacturer has innovative technologies in the corresponding field.
...
## Summary
To address the difficulties in adapting and using ASICs, veScale aims to do more than just benchmark evaluation. We evaluate from the perspective of AI production scenarios, with actual business usage methods for evaluation, ensuring that the evaluation results can be directly reproduced in production scenarios. In this process, veScale not only acts as an evaluation suite but can also be used as a model conversion production tool.
...
- Website: https://volcengine.github.io/veScaleWeb/
